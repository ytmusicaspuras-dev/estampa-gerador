...
export const Generator: React.FC<GeneratorProps> = ({ user, setView }) => {
  ...
  const handleImageUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      if (file) {
          if (file.size > 5 * 1024 * 1024) {
              setErrorMsg("Imagem muito grande. Escolha uma menor.");
              return;
          }
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      if (!base64Image) throw new Error("Falha ao gerar imagem.");

      // 2. Tenta Salvar na Biblioteca (Agora lança erro se falhar)
      await saveToLibrary({
      ...
      // Transição segura
      setTimeout(() => setView('library'), 1000);

    } catch (e: any) {
      console.error("Erro no Gerador:", e);
      // Mensagem amigável para erro de cota
      if (e.message && e.message.includes("Memória")) {
          setErrorMsg("Memória cheia! Apague algumas artes antigas da biblioteca e tente novamente.");
      } else {
          setErrorMsg(e.message || "Não foi possível gerar ou salvar a imagem. Tente novamente.");
      }
    } finally {
